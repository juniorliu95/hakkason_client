...
class socket_client_module:
    ...
    def connect(self, image):
        if not self.socket_client.connect(self.ip, self.port, self.username, self.show_error):
            return

        self.socket_client.send(image)
        self.socket_client.listen(self.incoming_message, self.show_error)

    def incoming_message(self, username, message):
        # Update chat history with username and message, green color for username
        self.socket_client.received.append(message)
        return username
...
class CoolApp(App):
    def build(self):
        ...
        self.videobutton2 = self.vi_sc.btn2
        self.videobutton2.bind(on_press=self.vi2_callback)
    ...
    def vi2_callback(self, instance):
        self.video.source = './hourglass.mp4'
        #self.video.source = './1.mp4'
    ...
    def im_callback(self, instance):
        ...
        self.image_path = "./images/IMG_{}.png".format(timestr)
        self.video_path = "./videos/V_{}.mp4".format(timestr)
        self.im_sc.img.export_to_png(self.image_path)
        print("Captured!")
        ...
        while not len(scm.socket_client.received):
            socket_client.send('hello')

        # deal with videos
        with open(self.video_path, 'wb') as f:
            f.write(scm.socket_client.received[0])
        scm.socket_client.received = []
        
        scm.socket_client.client_socket.close()
